...
function MMDScene({
      ...
      if (bone) {
        bone.setRotationQuaternion(
          Quaternion.FromEulerAngles(
            boneRotation.axis === "x"
              ? boneRotation.value
              : bone.rotationQuaternion!.toEulerAngles().x,
            boneRotation.axis === "y"
              ? boneRotation.value
              : bone.rotationQuaternion!.toEulerAngles().y,
            boneRotation.axis === "z"
              ? boneRotation.value
              : bone.rotationQuaternion!.toEulerAngles().z
          ),
          Space.LOCAL
        );
      }
  ...
  useEffect(() => {
    if (materialVisible) {
      ...
      const mesh = mmdModelRef.current!.mesh.metadata.meshes.find(
        (m: Mesh) => m.name === materialVisible.name
      );
      if (material && mesh) {
  ...
  useEffect(() => {
    if (sceneRef.current) {
      if (enableSplitView) {
        ...
        sceneRef.current.activeCameras = [
          cameraRef.current!,
          topCamera,
          sideCamera,
          frontCamera,
  ...
  useEffect(() => {
    const createScene = async (canvas: HTMLCanvasElement): Promise<Scene> => {
      ...
      mmdWasmInstanceRef.current = await getMmdWasmInstance(
        new MmdWasmInstanceTypeMPD(),
        2
      );
      mmdRuntimeRef.current = new MmdWasmRuntime(
      ...
      mmdRuntimeRef.current!.onPauseAnimationObservable.add(() => {
        if (
          mmdRuntimeRef.current!.currentTime ==
          mmdRuntimeRef.current!.animationDuration
        ) {
  ...
  useEffect(() => {
    const loadMMD = async (): Promise<void> => {
      ...
      SceneLoader.ImportMeshAsync(
        ...
        mmdModelRef.current = mmdRuntimeRef.current!.createMmdModel(
          mesh as Mesh,
          {
            buildPhysics: {
              worldId: 0,
  ...
  useEffect(() => {
    ...
    const loadAnimation = async (): Promise<void> => {
      const vmd = await new VmdLoader(sceneRef.current!).loadAsync(
        "vmd",
        selectedAnimation
      );
      const animation = new MmdWasmAnimation(
        vmd,
        mmdWasmInstanceRef.current!,
        sceneRef.current!
      );
  ...
  useEffect(() => {
    ...
    const setBoneRotation = (
      bone: IMmdRuntimeLinkedBone | null,
      rotation: Rotation
    ): void => {
      if (!bone) return;
      bone.setRotationQuaternion(
    ...
    const updateMMDPose = (mmdModel: MmdWasmModel | null, body: Body): void => {
      ...
      const morphTargets = {
        まばたき: Math.max(
          Math.pow(1 - result.right_eye_openness, 1.5),
          Math.pow(1 - result.left_eye_openness, 1.5)
        ),
  ...
  const handleCaptureScreenshot = () => {
    ...
    CreateScreenshotAsync(engineRef.current!, cameraRef.current!, {
      precision: 1,
    }).then((b64) => {
  ...
  function createVMD(frames: RecordedFrame[]): Blob {
    ...
    const totalSize =
      headerSize +
      4 +
      boneFrameSize * frameCount * boneCnt +
      4 +
      morphFrameSize * frameCount * morphCnt +
      4 +
      4 +
      4;
    ...
    for (let i = 0; i < 30; i++) {
      dataView.setUint8(
        offset + i,
        i < header.length ? header.charCodeAt(i) : 0
      );
    }
    offset += 30;
  ...
  return (
    <>
      ...
      <Tooltip title={!isRecording ? "Start recording" : "Stop recording"}>
        ...
        >
          {!isRecording ? (
            <MicOff sx={{ width: "26px", height: "26px" }} />
          ) : (
            <Mic sx={{ width: "26px", height: "26px" }} />
          )}
        </IconButton>
      </Tooltip>
